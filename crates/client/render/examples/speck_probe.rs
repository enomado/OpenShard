...
fn sweep(lo: [f32; 3], hi: [f32; 3], steps: u32) -> (u32, u32, Vec<(f32, f32, usize)>) {
    let mut total = 0;
    let mut sides = 0;
    ...
    for ix in 0..=steps {
        for iy in 0..=steps {
            ...
            let x = lo[0] + (hi[0] - lo[0]) * fx;
            let y = lo[1] + (hi[1] - lo[1]) * fy;
            ...
            let back = 4.0;
            let from = [x - back * VIEW[0], y - back * VIEW[1], hi[2] - back * VIEW[2]];
            let met = impostor::meets(from, lo, hi);
            total += 1;
            let axis = axis_of(met.normal);
            if axis != 2 {
                sides += 1;
                if where_.len() < 12 {
                    where_.push((x - lo[0], y - lo[1], axis));
                }
            }
        }
    }
