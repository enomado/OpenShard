...
use openshard_protocol::{encode_gump_display, GumpButton, GumpLayout, GumpResponse};
...
pub fn open(state: &mut WorldState, player: EntityId, context: CraftGumpContext) {
    ...
    let close = openshard_protocol::encode_close_gump(CRAFT_GUMP, 0);
    let packet = encode_gump_display(serial.raw(), CRAFT_GUMP, WINDOW_X, WINDOW_Y, string, lines);
...
pub fn close(state: &mut WorldState, player: EntityId) {
    ...
    if let Some(&Client { connection, .. }) = state.registry.get::<Client>(player) {
        let packet = openshard_protocol::encode_close_gump(CRAFT_GUMP, 0);
        state.send(connection, packet);
    }
}
