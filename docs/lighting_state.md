...
| 🟡 | **Specks and dashes on an indoor floor — and now on a table's own top** | Furniture drawn wider than its own per-tile box; the pixel over the boundary belongs to a static whose box is a tile away, and its ray leaves through a side face. **Diagnosed 2026-08-10 off two client F12 dumps: it is the absence of a name.** Where counters abut at one `z`, the boundary row of the **lid** is answered with a side face — 464 pixels of one frame and 270 of another, 442 and 268 of them pegged to a tile edge, `shadow` and `reach` identical across the line while `flames` goes 15×. Not the fringe (F2 moves the picture and not these), not a tie at the top edge (the quantum repair reaches 46 of a body's 1,010 side-face fragments and none of the dashes), and **`SOLID_NOBODY` in 270 of 270**: these pieces are `CLEAR`, so nothing names them, so `merge::merged` never folds the run — and an unnamed run of N counters is N boxes with naked interior faces where a named one is a single box with none. `docs/parity.md` P4 step 2 owns the repair; see the rebuild's seam-probe entry for the numbers |
