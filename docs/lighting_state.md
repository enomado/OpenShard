...
| 🚩 | **Bright strips lie across a staircase's steps** | One flight, and its boxes disagree about which way it climbs: the six graphics at `(1454, 1728)` are fitted `prism N`, `W`, `E`, `E`, a plain box and none at all. The two side families come out the same size (4,957 east, 4,927 south of 35,100 fragments) and at right angles to each other — the south faces run along the steps the art drew, the east faces across them. The lamp stands east, so the crosswise family is the only one lit (flame 165.4 against 12.7) and draws as strips over the treads. Not the ground (`Kind` says static), not the art (flat light draws a plain grey staircase), not the height, not the walk, not the merge. `best_prism` scores each picture alone and a flight is a run |
