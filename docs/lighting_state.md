...
| 🟡 | **Specks and dashes on an indoor floor — and now on a table's own top** | Furniture drawn wider than its own per-tile box; the pixel over the boundary belongs to a static whose box is a tile away, and its ray leaves through a side face. 32 of 66 are pieces the grid holds nothing for, so no identity can excuse them. **Reported again 2026-08-10 on the furniture itself** (client F12 dump, eye tile `(1496, 1659)`, `1919x2077`): where counters abut at one `z`, the boundary row of the **lid** is answered with a side face — 464 pixels of the frame, 442 pegged to a tile edge, `shadow` and `reach` identical across the line while `flames` goes 15×. `shows_a_side` ended this for a *floor*, whose lid is `LID_THICKNESS` tall; a table is a body, so its side face is tall enough to be handed out. See the rebuild's seam-probe entry |
