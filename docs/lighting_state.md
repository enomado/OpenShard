...
| Billboards (mobiles) | 🟡 half | the inflated-silhouette normal and the choice between it and the camera-facing plane — its *done when* is a person looking; plus a live anchor defect (below) | phase 7 |
...
| 🚩 | **Vertical stripes on a walking mobile** | Two anchors for one position: the sprite is drawn at the eased sub-tile position, `billboard_at` takes the destination tile. Mid-step they are up to a whole tile apart, and a billboard's screen column is one `(x, y)` so every shadow boundary across it is vertical |
