...
| 🚩 | **A staircase has no shading of its own** | A climbable is fitted as a `Prism` and then every tread is handed `Edges::ANY` — a mask `boxes_of` writes to pick an *occlusion test* and the normal rule reads as *the art named no face*. A tread's lid and its riser are planes the art did draw, and `Prism::mesh` still knows both. Reported at `(1454, 1728)`; measured in the rebuild's backlog, where it is one field with two domains rather than a fourth answer to the body question |
